...
**    > Created Time: 2018??05??17?? ?????? 16ʱ52??44??
*************************************************************************/
...
int create_server(char *ip, short port, int num)
{
    ...
    if (bind(listen_fd, (struct sockaddr *)&server_addr, sizeof(struct sockaddr_in6)) < 0)
    {
        ERR_PRINT("bind");
        ret = ECALL;
        goto failure;
    ...
    while (1)
    {
        ...
        if (NULL == inet_ntop(AF_INET6, &client_addr.sin6_addr, buf, sizeof(buf)))
        {
            ERR_PRINT("inet_ntop");
            ret = ECALL;
            //goto failure;
